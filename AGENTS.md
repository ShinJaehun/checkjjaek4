...
- `Jjaek`는 optional 문맥으로 특정 책, 특정 짹, 특정 사용자를 가리킬 수 있다.
- 이 optional 문맥은 nullable foreign key로 표현할 수 있다.
...
- 특정 사용자를 향한 `Jjaek`를 profile-context Jjaek으로 해석할 수 있다.
- `books/:id`는 서비스 전체의 절대 중심 화면이 아니라,
  **책 정보와 그 책에 연결된 책짹을 다루는 책 문맥 화면**이다.
- `users/:id`는 관계에 따라 그 사람의 책과 짹을 보고 상호작용하는
  **사람 문맥 화면**이다.
- `users/:id`의 입력창은 DM이 아니라 profile-context Jjaek 작성 진입으로 본다.
- profile-context Jjaek은 작성자의 글이며, 대상 사용자를 향한 공개/관계기반 짹이다.
