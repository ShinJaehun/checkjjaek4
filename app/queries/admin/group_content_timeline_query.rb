module Admin
  class GroupContentTimelineQuery
    ...
    def apply_search
      ...
      @jjaek_scope = @jjaek_scope.joins(:user).where(
        "jjaeks.content ILIKE :term OR users.name ILIKE :term OR users.email ILIKE :term",
        term: pattern
      )
      @comment_scope = @comment_scope.joins(:user).where(
    ...
    def count_sql
      "SELECT COUNT(*) FROM (#{union_sql}) admin_group_content_timeline"
    end
    ...
    def page_sql
      ...
      <<~SQL.squish
        ...
        FROM (#{union_sql}) admin_group_content_timeline
