...
관계 MVP의 세부 기준은
`docs/specs/social_relationships_mvp.md`를 함께 따른다.
특히 `Follow` / `BookFriendship`, `/relationships`,
관계 Notification 진입점, Comment visibility 정책은
새 문서를 기준으로 정리한다.
...
- 현재 MVP에는 별도 책장 모델을 두지 않지만,
  후속 단계에서는 **한 서재 안 여러 책장** 구조로 확장할 수 있어야 한다.
...
- `users/:id`의 읽기 범위는 관계에 따라 다르게 둔다.
  - 아무 관계 없음: 공개 책장에 담긴 책 목록만
    - 공개 짹은 보지 않는다
    - 책친구 공개 짹은 보지 않는다
    - profile-context 작성 진입은 보지 않는다
...
### 한 서재 안 여러 책장 지원은 후속 단계로 둔다

초기 MVP에서는 사용자마다 기본 서재 1개를 전제로 유지한다.
이번 단계에서는 타인 프로필의 현재 `BookshelfEntry` 목록 전체를
"기본 공개 책장"으로 임시 해석한다.
이 규칙은 여러 책장/책장 공개범위 모델 도입 전까지의 MVP 임시 규칙이다.

후속 단계에서는:
- 한 사용자가 여러 책장을 가질 수 있게 한다.
- 각 책장은 공개/비공개를 선택할 수 있게 한다.
- 다른 사용자의 프로필에서 보이는 책 목록은 우선 공개 책장 기준으로 해석한다.
- 이 확장 단계에서 `Bookshelf`, `BookshelfMembership` 구조를 함께 검토한다.
...
10. 한 서재 안 여러 책장과 책장 공개/비공개는 후속 확장으로 남긴다.
11. 그룹, 학생 계정 전환 흐름은 후속 확장으로 남긴다.
