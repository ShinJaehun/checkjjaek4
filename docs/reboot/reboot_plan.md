...
- 특정 사용자 문맥도 Jjaek에 선택적으로 연결될 수 있다.
...
- 홈에서 일반 짹 작성
- 특정 책에 책짹 작성
- 특정 사용자 프로필 문맥에서 profile-context 짹 작성
- 공개 범위 선택
- Jjaek 상세
- 댓글/좋아요 연결
...
원칙:
- 홈은 일반 짹 흐름의 중심 화면이다.
- users/:id 입력창은 DM이 아니라 profile-context Jjaek 작성 진입이다.
- profile-context Jjaek은 작성자의 글이며, 대상 사용자 문맥을 가진다.
- 작성자는 자신의 홈 피드에서 이 글을 보고, 대상 사용자도 자신의 홈 피드에서 이 글을 볼 수 있다.
...
초기 단계에서 `Jjaek`는 optional foreign key를 통해 문맥을 가질 수 있다.

- `book_id`: 책 문맥
- `quoted_jjaek_id`: 인용/응답 문맥
- `target_user_id`: 특정 사용자 문맥

이 값들은 nullable로 둘 수 있으며,
값이 있을 때의 허용 규칙은 policy / request spec으로 고정한다.
...
- 프로필(`users/:id`)은 관계에 따라 그 사람의 책과 짹을 보고,
  필요하면 공개/관계기반 짹 작성 진입을 둘 수 있는 사람 문맥 화면으로 두고
