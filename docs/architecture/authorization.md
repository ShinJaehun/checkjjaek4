...
- Library는 책장 구조가 있는 깊은 서재 화면으로 해석한다.
- 로그인 사용자는 타인 프로필의 공개 가능한 콘텐츠를 볼 수 있다.
- `stranger / follow`는 프로필에서 `public` 책 목록을 flat summary 형태로 볼 수 있다.
- `stranger / follow`는 프로필에서 책장 탭, 책장 정렬 UI, 책장 관리 UI, 책 이동 UI, Library 링크를 볼 수 없다.
