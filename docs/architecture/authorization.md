...
- 프로필/Library 책 목록은 `BookshelfPolicy` / `BookshelfEntryPolicy::ProfileScope`의 Bookshelf visibility 기준을 따른다
- Bookshelf visibility는 `self`: 전체, `book_friend`: `public` + `book_friends`, `stranger/follow`: `public`만 허용한다
- 프로필/Library 책장 탭도 접근 가능한 Bookshelf만 표시한다
...
- `self / book_friend`는 프로필 책 목록에서 상태와 스티커를 볼 수 있다
- Bookshelf 생성/수정/삭제/순서 변경은 owner만 가능하며, 새 Bookshelf는 현재 사용자의 Bookshelf로만 생성한다
- 기본 Bookshelf는 수정/삭제/순서 변경할 수 없고, 책이 들어 있는 일반 Bookshelf는 삭제할 수 없다
- `BookshelfEntry`의 책장 이동은 owner만 가능하며, 대상 Bookshelf도 현재 사용자의 Bookshelf여야 한다
