...
- **동아리 운영 정지 / 동아리 운영 복구**는 global admin이 active Group의 `Group#operation_suspended_at`으로 Group 자체의 새 운영 mutation을 제한·복구하는 기능이다. 아직 승인되지 않은 `pending_approval` Group은 운영 정지·복구 대상이 아니며 기존 승인·거절 심사 lifecycle만 따른다. pending 상태에는 운영 정지 action을 노출하지 않고 policy와 상태 변경 service를 포함한 server-side에서도 직접 요청을 거부한다. group admin의 기존 자발적 운영 종료 lifecycle과 별도이며 Group 대상 append-only `ModerationAction`에 사유와 복구 연결을 남긴다.
...
- `pending_approval` Group에는 운영 정지 action이 노출되지 않는다.
- `pending_approval` Group의 운영 정지·복구 직접 요청은 server-side에서 거부된다.
- active Group은 기존 동아리 운영 정지·복구 정책을 유지한다.
