...
- Library 보기 모드
- SortableJS group 기반 책장 간 Drag and Drop 이동
- 일반 책장 생성 UI
- 일반 책장 수정/삭제 UI
- 책장 `color_key` 표시와 선택
- 일반 책장 위/아래 순서 변경 UI
...
12. 본인은 SortableJS group 기반 Drag and Drop으로 책을 다른 내 책장으로 이동할 수 있다.
...
현재 구현은 현재 선택된 책장 list와 이동 대상 책장 list 두 개에만 SortableJS group을 적용하는 책장 간 이동을 제공한다.
...
- 책 카드를 이동 대상 책장 list에 놓으면 기존 책장 간 이동 흐름을 사용한다.
- 책장 간 이동은 move endpoint, 같은 책장 안 reorder는 reorder endpoint를 사용한다.
- detail/compact 카드 전체는 `manual` 정렬에서 SortableJS drag source로 동작한다.
...
- `cross_shelf_sortable_controller`: 선택된 책장과 이동 대상 책장 사이의 이동, 그리고 두 list 내부의 manual reorder
- `bookshelf_entries_sort_controller`: 이동 대상 책장이 없는 단일 list manual reorder fallback
