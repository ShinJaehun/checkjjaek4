...
- 카드 안의 링크, 버튼, select, form, details/summary 같은 interactive 요소에서는 drag가 시작되지 않도록 guard한다.
...
책 이동 select fallback은 카드 안의 접이식 영역에 두고, 기존 select 기반 fallback 자체는 삭제하지 않는다.
...
- publisher와 긴 sticker 목록은 compact하게 줄여 표시한다.
- 책장 이동은 DnD를 기본 흐름으로 두되, 기존 select fallback은 접이식 영역에 둔다.
- 책 카드 안의 링크, 버튼, select, form 같은 interactive 요소는 full-card drag 대상에서 제외한다.
