...
- sort가 `manual`이고 owner 화면일 때만 책 카드 전체에 가까운 영역을 reorder handle로 사용한다.
- 작은 “순서 변경” 버튼은 두지 않는다.
- 카드 전체에 가까운 reorder handle을 같은 책장 안에서 드래그하면 카드 순서를 바꾸고 기존 reorder endpoint로 저장한다.
- 카드 전체에 가까운 reorder handle을 책장 tab/drop target에 놓으면 기존 책장 간 이동 흐름을 사용한다.
- 카드 전체에 가까운 reorder handle을 책장 tab/preview panel에 놓는 경우 reorder 요청은 보내지 않는다.
...
- 책장 tab, preview panel, dropzone은 유효한 책 이동 drag에만 반응한다.
- 링크, 버튼, details, summary, select, input, form 같은 interactive 요소는 reorder drag 시작 대상에서 제외한다.
...
4. 책장 안 책 순서 변경 1차를 유지한다.
   ...
   - 모바일/접근성 fallback 검토
5. Library UX redesign 1차를 적용한다.
   - 책장 tab/index와 선택된 책 목록 영역의 시각적 연결
   - compact book card 방향 정리
   - 정렬/새 책장/책장 관리 UI 분리
   - full-card drag는 2차로 보류
...
### Library UX redesign 1차 색상 원칙

- 1차에서는 책 목록 전체 배경을 강하게 칠하지 않는다.
...
Library UX redesign 1차에서는 책 이동 select를 카드 안에 그대로 노출하지 않고, 더보기 메뉴, 설정 영역, 또는 접이식 관리 패널로 옮기는 방향을 검토한다.
다만 기존 select 기반 fallback 자체는 바로 삭제하지 않는다.
...
Library 정리 화면의 책 카드는 책을 빠르게 훑고 옮기는 데 집중한다.

- 표지, 제목, 저자, 상태/별점 정도를 중심으로 compact하게 표시한다.
- publisher, 긴 sticker 목록, 카드 내부 책장 이동 select는 1차 compact 대상이다.
- 책장 이동은 DnD를 기본 흐름으로 두되, 기존 select fallback은 더보기 메뉴, 설정 영역, 또는 접이식 관리 패널로 옮기는 방향을 검토한다.
- 책 카드 안의 링크, 버튼, select, form 같은 interactive 요소는 향후 full-card drag와 충돌하지 않도록 분리한다.
