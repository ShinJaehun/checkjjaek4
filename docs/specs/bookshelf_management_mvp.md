...
이 문서는 책장 관리 기능을 단계별로 확장하기 위한 기준 문서다.
...
일반 책장 생성 UI는 이 MVP 범위에 포함한다.
책장 수정/삭제, 책장 색상 선택, drag & drop 이동은 후속 작업으로 남긴다.
...
책장 생성 UI와 함께 이 제한을 실제 validation으로 추가한다.
기본 책장 1개가 이미 존재하므로 사용자가 추가로 만들 수 있는 일반 책장은 최대 19개다.
...
책장 탭과 책 이동 UI를 기반으로, 사용자는 본인 프로필에서 일반 책장을 새로 만들 수 있다.
...
색상, 정렬 순서, 설명 문구는 이번 단계에서 입력받지 않는다.
...
1. 일반 책장 이름 수정
2. 일반 책장 visibility 변경
3. 일반 책장 삭제
4. 책장 삭제 시 안의 책을 기본 책장으로 이동
5. 책장 `color_key` 추가
6. 책장 색상 선택 UI
7. drag & drop 기반 책 이동
8. 책장 순서 변경
