...
이 문서는 checkjjaek4에서 `BookActivity`를 도입하기 전,
책 관련 활동을 어떤 기준으로 기록하고 피드에 노출할지 정리한다.

`BookActivity`는 현재 구현된 모델이 아니다.
이 문서는 구현 전 canonical spec 초안이다.

현재 checkjjaek4의 피드는 `Jjaek` 중심으로 동작한다.
`BookActivity`는 후속 단계에서 `BookshelfEntry`와 `BookshelfEntrySticker`의 변화를
피드에서 소비하기 좋은 이벤트로 표현하기 위한 모델이다.
...
초기 `BookActivity` MVP에서는 아래 이벤트만 후보로 둔다.

### 1. 책을 서재에 담음

사용자가 검색 결과나 책 상세 화면에서 책을 자기 서재에 처음 담았을 때 생성할 수 있다.
...
사용자가 서재 항목의 대표 상태를 다른 값으로 변경했을 때 생성할 수 있다.
...
- 상태 변경 전후 값을 기록할지 여부는 구현 단계에서 결정한다.
- 최소 MVP에서는 변경 후 상태만 저장해도 된다.

### 3. 대표 상태를 비움

사용자가 기존 대표 상태를 비워 상태 없음으로 만들었을 때 생성할 수 있다.
...
- 여러 스티커를 한 번에 추가한 경우, 활동을 하나로 묶을지 스티커별로 만들지는 구현 전 결정한다.
- MVP에서는 스티커별 이벤트로 시작하는 편이 단순하다.

### 5. 스티커를 제거함

사용자가 책에서 기존 스티커를 제거했을 때 생성할 수 있다.
...
- 여러 스티커를 한 번에 제거한 경우, 활동을 하나로 묶을지 스티커별로 만들지는 구현 전 결정한다.
- MVP에서는 스티커별 이벤트로 시작하는 편이 단순하다.
...
`BookActivity`는 후속 구현에서 home feed 또는 profile feed에 노출될 수 있다.
...
- 타인 프로필에서는 관계에 따라 해당 사용자의 BookActivity를 볼 수 있다.
- 관계 없는 사용자는 공개 가능한 BookActivity만 본다.
- 소식받기만 한 사용자는 공개 가능한 BookActivity만 본다.
- 책친구는 공개 가능한 BookActivity와 책친구 범위 BookActivity를 볼 수 있다.

다만 이 규칙은 여러 책장/책장 공개 범위 도입 전까지 임시 정책이다.

### 책 상세 후보 규칙

`books/:id`에서 BookActivity를 노출할지는 MVP 구현 시 별도로 결정한다.
...
### Policy / scope spec 후보

- 사용자는 자기 BookActivity를 볼 수 있다.
- 소식받는 사용자의 공개 가능한 BookActivity를 볼 수 있다.
- 책친구의 책친구 범위 BookActivity를 볼 수 있다.
...
- BookActivity 생성 실패가 BookshelfEntry 저장 성공을 깨뜨리지 않도록 할지 여부는 구현 전 결정한다.
...
- home feed 후보 scope
- profile feed 후보 scope
- 관계 기반 접근 범위 테스트

### Step 3. 피드 표시

- Jjaek 중심 피드에 BookActivity를 섞을지 별도 섹션으로 둘지 결정
- 초기에는 별도 섹션 또는 단순 timeline item으로 시작

### Step 4. 문서 갱신

구현이 완료되면 아래 문서를 함께 갱신한다.
...
- `docs/reboot/reboot_plan.md`
  - BookActivity가 현재 계획 단계임을 추적
- `docs/architecture/current_system.md`
  - 현재 구현에는 BookActivity가 없음을 확인
