...
일반 책장은 비어 있을 때만 삭제할 수 있다.
책이 들어 있는 일반 책장을 삭제할 때 BookshelfEntry를 기본 책장으로 자동 이동하지 않고,
책장과 함께 BookshelfEntry를 같이 삭제하지도 않는다.
사용자가 직접 책을 다른 책장으로 옮기거나 제거해서 책장을 비운 뒤 삭제해야 한다.
...
본인 Library의 기본 책 목록 정렬은 `manual`이며, visitor / book_friend 등 타인이 보는 Library의 기본 정렬은 `recent`이다.
