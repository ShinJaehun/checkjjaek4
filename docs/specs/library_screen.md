...
Library Screen의 기본 경로는 다음을 우선 검토한다.
...
또는 기존 구조가 더 단순하면 다음도 가능하다.

```text
LibrariesController#show
```

다만 사용자별 하위 화면이라는 의미가 강하므로 `Users::LibrariesController`를 우선 검토한다.
...
Library Screen의 조회 범위는 기존 Bookshelf visibility 정책을 따른다.
...
Library Screen 도입 spec 단계에서는 다음을 하지 않는다.
...
- 기존 Bookshelf visibility / status / sticker 정책 유지
- 프로필에는 “서재 보기” 링크 추가
...
- 프로필에 기본 책장 또는 최근 책 몇 권만 표시할지 결정
- “전체 서재 보기” 링크 제공
- visitor 권한별 요약 노출 범위 정리
...
3. accepted book_friend는 접근 가능한 책장을 볼 수 있다.
4. stranger / follow-only는 public 책장만 볼 수 있다.
