...
Bookshelf 기능은 다음 범위까지 확장되었고, 현재 책장 구조와 관리 기능의 주 화면은 Library Screen이다.
...
따라서 프로필은 공개 요약 화면으로 가볍게 유지하고, 사용자의 책장/책 목록 관리 기능은 Library Screen에서 제공한다.
...
`/bookshelf_entries`는 더 이상 대표 서재 화면으로 사용하지 않는다.

대표 서재 화면은 `/users/:user_id/library`다.

다만 `BookshelfEntry` 자체의 생성/수정/이동/삭제 흐름은 유지한다.

- 책 검색이나 책 상세에서 책을 서재에 담을 때 `BookshelfEntry`를 생성한다.
- 책 상태와 스티커를 수정할 때 `BookshelfEntry`를 수정한다.
- Library 안에서 책을 다른 책장으로 옮길 때 `BookshelfEntry`의 `bookshelf_id`를 변경한다.
- 서재에서 책을 제거할 때 `BookshelfEntry`를 삭제한다.
...
현재 구현은 프로필 화면과 Library Screen의 역할을 나눈다.
...
Library Screen은 Bookshelf UI의 주 화면이다.
...
- 사용자 정보
- 공개 가능한 책 목록 요약
- 최근 활동: visible Jjaek + visible BookActivity
- self / accepted book_friend에게만 “서재 보기” 링크

프로필 화면에서 제거하거나 축소한 것:
...
현재 Library Screen 정리 범위에서는 다음을 하지 않는다.
...
1. `/users/:user_id/library` route와 `Users::LibrariesController#show`가 Library Screen을 제공한다.
2. Library Screen은 책장 탭, 책장별 책 목록, 책 이동, 책장 생성/수정/삭제, 색상, 순서 변경, 책 목록 정렬을 제공한다.
3. Library Screen 접근은 `self / accepted book_friend`에게만 허용한다.
4. `stranger / follow`가 Library Screen에 접근하면 프로필 화면으로 redirect된다.
5. 프로필 화면은 요약 화면으로 정리되었고, 책장 탭/정렬/관리/책 이동 UI를 제공하지 않는다.
6. 프로필 화면에는 공개 가능한 책 목록 요약과 self / accepted book_friend 대상 “서재 보기” 링크만 남긴다.
7. `/bookshelf_entries` index 화면과 route는 제거되었고, `BookshelfEntry`의 생성/수정/이동/삭제 흐름만 유지한다.
...
Library Screen 관련 최소 테스트 기준은 다음과 같다.
...
1. self / accepted book_friend 프로필 화면에 Library Screen으로 이동하는 링크가 보인다.
